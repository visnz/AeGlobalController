...
function UI(object){
        ...
        "textme: StaticText {text:'桶子工具箱'},"+
            ...
            "thirdGroup: Group {text:'功能', alignment:['fill','top'], orientation:'column', spacing:5 ,"+
                ...
                "line4: Group {text:'功能', alignment:['fill','top'], orientation:'row', spacing:5 ,"+
                    ...
                    // "button42: Button {text:'快糊', alignment:['fill','top'], preferredSize:[30,25]},"+
                    // "button43: Button {text:'百叶', alignment:['fill','top'], preferredSize:[30,25]},"+
                    // "button44: Button {text:'线渐', alignment:['fill','top'], preferredSize:[30,25]},"+
            ...
            "fouthGroup: Group {text:'功能', alignment:['fill','top'], orientation:'column', spacing:5 ,"+
                ...
                "line1: Group {text:'motion2', alignment:['fill','top'], orientation:'row', spacing:5 ,"+
                    ...
                    "s3: Slider {text:'easeOut',alignment:['fill','top'], preferredSize:[30,25]}"+
                    "s2: Slider {text:'ease',alignment:['fill','top'], preferredSize:[30,25]}"+
                    "s1: Slider {text:'easeIn',alignment:['fill','top'], preferredSize:[30,25]}"+
                "},"+
                "line2: Group {text:'keyfast', alignment:['fill','top'], orientation:'row', spacing:5 ,"+
                    ...
                    "outfun3: Button {text:'ScIn', alignment:['fill','top'], preferredSize:[30,25]},"+
                    "outfun4: Button {text:'ScOut', alignment:['fill','top'], preferredSize:[30,25]},"+
    ...
    myPalette.grp.thirdGroup.line2.buttonN1.onClick=function buttonN1(){addEffects("色相/饱和度")}
    ...
    myPalette.grp.thirdGroup.line4.button41.onClick=function button41(){addEffects("投影")}
...
//============ outfun ==========//
    ...
    function OF_Keyfast_move(direct){
        ...
        log("OF_Keyfast_move, "+direct)
        for (i = 0; i < myLayer.length; i = i + 1) {
            if (direct == 3 || direct == 2) {
                slideDirection(i, myTime, slideAmountX, slideAmountY, undefined, myLayer, moveDirection);
            ...
            if (myLayer[i].property("ADBE Transform Group").property("ADBE Position").dimensionsSeparated == false && linearCheckBoxAmount == "true") {
    ...
    function slideDirection(num, time, slideX, slideY, command, layer, direction) {
        ...
        if (layer[num].property("ADBE Transform Group").property("ADBE Position").dimensionsSeparated == false && direction == 1 || direction == -1) {
            positionValue = layer[num].property("ADBE Transform Group").property("ADBE Position").value;
            position = layer[num].property("ADBE Transform Group").property("ADBE Position");
            if (position.numKeys >= 1 || command == 1) {
                position.setValueAtTime(time, positionValue);
                position.setValueAtTime(time + myTimeBonusAmount, positionValue + [slideX * direction, slideY]);
            } else {
                position.setValueAtTime(time, positionValue + [-slideX * direction, slideY]);
                position.setValueAtTime(time + myTimeBonusAmount, positionValue);
            }
        }
        ...
        if (layer[num].property("ADBE Transform Group").property("ADBE Position").dimensionsSeparated == false && direction == 2 || direction == 3) {
            ...
            if (position.numKeys >= 1 || command == 1) {
                position.setValueAtTime(time, positionValue);
                position.setValueAtTime(time + myTimeBonusAmount, positionValue + [slideX, slideY * directionMultiple]);
            } else {
                position.setValueAtTime(time, positionValue + [slideX, -slideY * directionMultiple]);
                position.setValueAtTime(time + myTimeBonusAmount, positionValue);
            }
        }
